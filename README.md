...
ข้อมูลที่ใช้: `bangkok_traffic_2560_Present (NotFinal) (5).xlsx` ชีต `traffic_data` วันที่สำรวจ 2017-03-01 ถึง 2026-05-29 (วัน–เดือน–ปีที่แสดงในตารางใช้ ค.ศ.) ข้อมูลต้นทางยังเป็นฉบับกำลังตรวจแก้ การเรียงเนื้อหาพร้อมนำเสนอไม่ได้หมายถึงรับรองความถูกต้องครบทุกแถว
...
1. **ปริมาณรถของหน่วยเดิมในสามช่วง:** ก่อนโควิด 100 → ระหว่างโควิด 91.5 → หลังโควิด 90.2 | จำนวน 721 หน่วย ค่านี้เป็นค่ามัธยฐานของดัชนีรายหน่วย ไม่ใช่ยอดรถรวมทั้งเมือง
2. **แต่ละพื้นที่เปลี่ยนไม่เหมือนกัน:** หลังโควิด มี 250/721 หน่วย (34.7%) ที่ไม่น้อยกว่าฐาน จึงต้องดูการกระจายและกรณีศึกษาร่วมกับภาพรวม
3. **องค์ประกอบรถเปลี่ยนด้วย:** รถยนต์รวมแท็กซี่: 71.72% → 72.51% → 73.39% | จำนวน 719 สัดส่วนที่เพิ่มขึ้นไม่ยืนยันว่าจำนวนรถเพิ่ม หรือคนเปลี่ยนรูปแบบเดินทาง
...
**คำถาม:** เรามีข้อมูลพอเปรียบเทียบข้ามช่วงหรือไม่ และเดือนไหนขาด?

**วิธีอ่านแกน สี และหน่วย:** แกนนอนคือเดือนรายงาน 1–12 แกนตั้งคือปี ค.ศ. สีเข้มหมายถึงมีแถวสำรวจมาก ตัวเลขแต่ละช่องคือจำนวนแถวในชุดที่ผ่านเกณฑ์ ไม่ใช่จำนวนคันรถ หนึ่งวันอาจมีหลายแยก หลายถนน และหลายช่วงเวลา จึงนับเป็นหลายแถวได้

**วิธีคำนวณและฐานเปรียบเทียบ:** นับแถวแยกตามปีและเดือนรายงาน ก่อนจำกัดเฉพาะหน่วยที่จับคู่ครบสามช่วง ช่องศูนย์หมายถึงไม่มีแถวในไฟล์ที่นำมาวิเคราะห์ ไม่ได้หมายความว่าวันนั้นไม่มีรถ ปีล่าสุดเป็นข้อมูลบางเดือนเท่านั้น

**ผลจากไฟล์รุ่นนี้:** วันสำรวจ 2017-03-01 ถึง 2026-05-29 | 17,440 แถว → ผ่านเกณฑ์ 17,250 แถว

**ความหมายและข้อควรระวัง:** กราฟนี้ใช้กำหนดวิธีเปรียบเทียบ: หากบางปีมีแถวมากกว่า การบวกยอดรถทั้งปีจะได้ผลที่ปนกับความเข้มข้นของการสำรวจ จึงต้องใช้หน่วยเดิมและแปลงเป็นคันต่อชั่วโมง ข้อมูลเป็นวันสำรวจบางวัน และปีล่าสุดไม่ครบปี; ช่อง 0 คือไม่มีรายการในไฟล์

**ตัวอย่างบทพูดหน้าชั้น:** “เรามีข้อมูลหลายปี แต่ไม่ได้มีทุกแยกทุกวัน จึงรวมยอดรายปีมาเทียบตรง ๆ ไม่ได้”

**นำไปใช้ต่อ:** เปรียบเทียบหน่วยเดิมและเวลาเดิมก่อนตีความ

**ควรนำเสนอหรือไม่:** ใช้เกริ่นสั้น ๆ ก่อนผลหลัก ไม่ต้องอ่านครบทุกช่อง

[เปิดตารางตัวเลขของกราฟ 01](tables/coverage.csv)

### กราฟ 02 — ปริมาณรถในหน่วยที่จับคู่ครบสามช่วง

![กราฟ 02: ปริมาณรถในหน่วยที่จับคู่ครบสามช่วง](BKK6/fig/02.png)

**คำถาม:** เมื่อเทียบสถานที่และเวลาเดิมครบสามช่วง ปริมาณรถอยู่ระดับใด?

**วิธีอ่านแกน สี และหน่วย:** แกนนอนคือก่อน ระหว่าง และหลังโควิด แกนตั้งเป็นดัชนี ไม่ใช่คันต่อชั่วโมง เส้นประ 100 คือฐานก่อนโควิด ดัชนี 90 หมายถึงอยู่ที่ 90% ของฐานของหน่วยนั้น หรืออยู่ต่ำกว่าฐาน 10%

**วิธีคำนวณและฐานเปรียบเทียบ:** รวมรถหกหมวดของแต่ละแถวแล้วหารชั่วโมงสำรวจ หาค่ามัธยฐานในแต่ละหน่วยและช่วงโควิด นำค่าของระหว่าง/หลังหารค่าก่อนของหน่วยเดียวกันคูณ 100 แล้วหาค่ามัธยฐานของดัชนีทุกหน่วยอีกครั้ง แต่ละหน่วยมีน้ำหนักเท่ากัน ไม่ได้ใช้ยอดรถรวมทั้งเมืองเป็นตัวหาร

**ผลจากไฟล์รุ่นนี้:** ก่อนโควิด 100 → ระหว่างโควิด 91.5 → หลังโควิด 90.2 | จำนวน 721 หน่วย

**ความหมายและข้อควรระวัง:** หากแท่งหลังโควิดต่ำกว่า 100 แปลว่าหน่วยกลางของชุดจับคู่ยังต่ำกว่าฐาน ไม่ได้แปลว่าทั้งกรุงเทพฯ มีรถลดเท่ากันทุกจุด และไม่ใช่เปอร์เซ็นต์ผลกระทบเชิงสาเหตุของโควิด เป็นดัชนีของตัวอย่าง ไม่ใช่ยอดรถรวมทั้งเมือง; ไม่พิสูจน์ผลเชิงสาเหตุ

**ตัวอย่างบทพูดหน้าชั้น:** “ตั้ง ก่อนโควิด ของแต่ละหน่วยเป็น 100 ผลภาพรวมคือ ก่อนโควิด 100 → ระหว่างโควิด 91.5 → หลังโควิด 90.2 | จำนวน 721 หน่วย จึงพบระดับต่ำกว่าฐานทั้งสองช่วง แต่ยังแยกสาเหตุไม่ได้ ช่วง 95%: ระหว่างโควิด 87.9–94.9; หลังโควิด 86.6–93.0”

**นำไปใช้ต่อ:** เริ่มดูการกระจายและสถานที่ เพราะค่ากลางอาจซ่อนความต่าง

**ควรนำเสนอหรือไม่:** กราฟหลักของข้อค้นพบที่ 1 ควรนำเสนอ

[เปิดตารางตัวเลขของกราฟ 02](tables/overview.csv)

### กราฟ 03 — แต่ละหน่วยอยู่เหนือหรือต่ำกว่าฐาน

![กราฟ 03: แต่ละหน่วยอยู่เหนือหรือต่ำกว่าฐาน](BKK6/fig/03.png)

**คำถาม:** ค่ากลางที่ลดลงเกิดเหมือนกันทุกหน่วยหรือไม่?

**วิธีอ่านแกน สี และหน่วย:** แต่ละจุดคือหนึ่งหน่วยทางแยก–ถนน–เวลา แกนนอนเป็นค่ามัธยฐานคันต่อชั่วโมงก่อนโควิด กราฟซ้ายแกนตั้งเป็นระหว่างโควิด กราฟขวาเป็นหลังโควิด จุดบนเส้นประหมายถึงเท่าฐาน จุดต่ำกว่าเส้นหมายถึงน้อยกว่าฐาน จุดเหนือเส้นหมายถึงมากกว่าฐาน

**วิธีคำนวณและฐานเปรียบเทียบ:** ใช้เฉพาะหน่วยที่จับคู่ครบสามช่วงและฐานเป็นบวก จุดหนึ่งจุดอาจมาจากหลายวันสำรวจที่สรุปด้วยค่ามัธยฐาน จึงไม่ใช่รถหนึ่งคันหรือวันหนึ่งวัน

**ผลจากไฟล์รุ่นนี้:** ต่ำกว่าฐาน: ระหว่างโควิด 64.5% | หลังโควิด 65.3% ของ 721 หน่วย

**ความหมายและข้อควรระวัง:** จุดทั้งเหนือและใต้เส้นแสดงว่าพื้นที่เปลี่ยนต่างกัน จุดที่มีปริมาณมากอาจโดดเด่นทางสายตา แต่การคำนวณดัชนีภาพรวมยังให้น้ำหนักแต่ละหน่วยเท่ากัน จุดซ้อนกันไม่ใช่หลักฐานว่าไม่มีความแตกต่าง จุดสำรวจไม่ได้เป็นตัวอย่างสุ่มทั้งกรุงเทพฯ; จุดบนแกนศูนย์ยังแสดงไว้

**ตัวอย่างบทพูดหน้าชั้น:** “เส้นทแยงคือระดับเท่าเดิม เรามีทั้งจุดที่สูงขึ้นและต่ำลง ไม่ใช่ทุกแห่งเปลี่ยนไปในทิศทางเดียวกัน”

**นำไปใช้ต่อ:** ไม่ใช้ค่ากลางแทนการตัดสินทุกทางแยก

**ควรนำเสนอหรือไม่:** เป็นกราฟเสริม ใช้เมื่ออาจารย์ถามว่าค่ากลางซ่อนอะไร หรือแทนกราฟ 4 หากเวลาน้อย

[เปิดตารางตัวเลขของกราฟ 03](tables/matched_levels.csv)

### กราฟ 04 — ค่ากลางซ่อนการกระจายมากแค่ไหน

![กราฟ 04: ค่ากลางซ่อนการกระจายมากแค่ไหน](BKK6/fig/04.png)

**คำถาม:** หลังโควิดมีกี่หน่วยที่ต่ำกว่า กลับเท่าฐาน หรือสูงกว่าฐาน?

**วิธีอ่านแกน สี และหน่วย:** แกนนอนเป็นช่วงดัชนี แกนตั้งเป็นจำนวนหน่วย ไม่ใช่จำนวนรถ แท่งสีส้มคือระหว่างโควิด สีเขียวคือหลังโควิด ช่วง 80–<100 หมายถึงตั้งแต่ 80 แต่ยังไม่ถึง 100 ส่วน ≥200 หมายถึงอย่างน้อยสองเท่าของฐาน

**วิธีคำนวณและฐานเปรียบเทียบ:** นำดัชนีแต่ละหน่วยจากกราฟ 2 มาจัดกลุ่ม แล้วนับจำนวนหน่วย ผลรวมแท่งของแต่ละช่วงโควิดต้องเท่ากับจำนวนหน่วยจับคู่ทั้งหมด กลุ่มตั้งแต่ 100 ขึ้นไปใช้รวมเพื่อบอกจำนวนหน่วยที่ไม่น้อยกว่าฐาน

**ผลจากไฟล์รุ่นนี้:** หลังโควิด มี 250/721 หน่วย (34.7%) ที่ไม่น้อยกว่าฐาน

**ความหมายและข้อควรระวัง:** เปอร์เซ็นต์หน่วยที่กลับเท่าหรือเกินฐานเป็นสัดส่วนของหน่วยสำรวจ ไม่ใช่สัดส่วนรถ ถนนทั้งหมด หรือประชากร จุดที่มีฐานต่ำอาจได้ดัชนีสูงมากจากจำนวนรถที่เพิ่มไม่มาก จึงต้องดูกราฟ 3 ประกอบเมื่อเลือกกรณีศึกษา ช่วงท้ายรวมดัชนีตั้งแต่ 200 ขึ้นไป; ฐานน้อยทำให้เปอร์เซ็นต์เปลี่ยนสูงได้

**ตัวอย่างบทพูดหน้าชั้น:** “ค่า หลังโควิด ใกล้ 90 ไม่ได้แปลว่าทุกแยกลด 10 เปอร์เซ็นต์ กราฟนี้แสดงทั้งกลุ่มต่ำกว่าฐานและกลุ่มที่กลับมาไม่น้อยกว่าฐาน”

**นำไปใช้ต่อ:** ใช้ข้อมูลคัน/ชั่วโมงประกอบดัชนีเมื่อเลือกจุดศึกษา

**ควรนำเสนอหรือไม่:** กราฟหลักของข้อค้นพบที่ 2 ควรนำเสนอ

[เปิดตารางตัวเลขของกราฟ 04](tables/index_distribution.csv)
